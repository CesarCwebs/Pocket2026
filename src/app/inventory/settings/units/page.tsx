...
import { Plus, Trash2, X, Package, FilePenLine, Loader2, ServerCrash } from "lucide-react";
...
function UnitModal({ onClose, onSave, unit, isSaving }: { onClose: () => void; onSave: (data: any) => Promise<void>; unit?: any, isSaving: boolean }) {
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
            <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} className="bg-white dark:bg-zinc-900 rounded-2xl w-full max-w-md border border-zinc-200 dark:border-zinc-800 shadow-xl">
                <header className="flex items-center justify-between p-4 border-b border-zinc-100 dark:border-zinc-800">
                    <h3 className="text-lg font-black text-zinc-800 dark:text-white">{unit ? 'Editar' : 'Nueva'} Unidad de Medida</h3>
                ...
                <div className="p-6 space-y-4">
                    <div>
                        <label className="text-xs font-bold text-zinc-500">ID Unidad (Abreviatura)</label>
                        <input type="text" value={id} onChange={e => setId(e.target.value)} disabled={!!unit} placeholder="PZA" className="w-full mt-1 p-2 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm disabled:opacity-50" />
                    </div>
                    <div>
                        <label className="text-xs font-bold text-zinc-500">Nombre Completo</label>
                        <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Pieza" className="w-full mt-1 p-2 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm" />
                    </div>
                    <div>
...
export default function UnitsSettingsPage() {
  ...
  const handleSave = async (data: any) => {
    ...
    const result = editingUnit
        ? await updateUnit(editingUnit.id, rest)
  ...
  const handleDelete = async (id: string) => {
      if (window.confirm('¿Estás seguro de que quieres eliminar esta unidad?')){
          ...
          if (result.success) {
            toast({ title: 'Unidad eliminada', description: result.message });
          } else {
  ...
  return (
    <InventoryLayout activeTab={activeTab} setActiveTab={setActiveTab}>
       <AnimatePresence>
        {isModalOpen && <UnitModal onClose={closeModal} onSave={handleSave} unit={editingUnit} isSaving={isSaving} />}
      </AnimatePresence>
      ...
      <header className="sticky top-0 z-30 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl border-b border-zinc-200 dark:border-zinc-800 px-8 h-20 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-black tracking-tight text-zinc-900 dark:text-white flex items-center gap-2">
            ...
            <span>Administrador de Unidades de Medida</span>
          </h1>
          <p className="text-sm font-semibold text-zinc-500 dark:text-zinc-400">Define las unidades para el control de stock.</p>
        </div>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-zinc-50 dark:bg-zinc-950/50 border-b border-zinc-100 dark:border-zinc-800">
                            <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">ID Unidad</th>
                    ...
                    <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                        {isLoading ? (
                            ...
                            <tr><td colSpan={5} className="text-center p-8 text-zinc-500 font-semibold">No hay unidades creadas todavía.</td></tr>
                        ) : (
                            units.map((unit) => (
                                <tr key={unit.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                                    <td className="px-6 py-4 font-mono text-sm text-blue-600 dark:text-blue-400">{unit.id}</td>
                                    <td className="px-6 py-4 text-sm font-bold text-zinc-800 dark:text-zinc-200">{unit.name}</td>
                                    <td className="px-6 py-4 text-sm text-zinc-500 dark:text-zinc-400">{unit.description}</td>
                                    <td className="px-6 py-4 text-sm text-zinc-500">{new Date(unit.createdAt).toLocaleDateString()}</td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex justify-end gap-2">
                                            <button onClick={() => openModal(unit)} className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors text-zinc-400 hover:text-blue-500"><FilePenLine className="w-4 h-4" /></button>
                                            <button onClick={() => handleDelete(unit.id)} className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors text-zinc-400 hover:text-rose-500"><Trash2 className="w-4 h-4" /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))
